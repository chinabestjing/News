...
<div id="right-content">
    <div class="top">
        <h2>资金管理 <small>商家提现图</small></h2>
    </div>
    <div class="clearfix mt10">
    	
		<form action="" method="get">
			<span class="fl" style="line-height: 33px;">选择月份：</span>
			<select name="month" id="" class="fl mr10" style="width: 50px; height: 33px; padding-left: 10px;">
				<?php for($i=1; $i < 13; $i++):?>
					<option value="<?php echo ($i); ?>" <?php if($month == $i): ?>selected="selected"<?php endif; ?>><?php echo ($i); ?></option>
				<?php endfor?>
			</select>

			<span class="fl" style="line-height: 33px;">选择商家：</span>
			<select name="uid" class="fl" style="width: 100px; height: 33px; padding-left: 10px;">
				<option value="0" <?php if($uid == 0): ?>selected="selected"<?php endif; ?>>全部</option>
				<?php if(is_array($shops)): foreach($shops as $key=>$v): ?><option value="<?php echo ($v['uid']); ?>" <?php if($uid == $v['uid']): ?>selected="selected"<?php endif; ?>><?php echo ($v["company"]); ?></option><?php endforeach; endif; ?>
			</select>

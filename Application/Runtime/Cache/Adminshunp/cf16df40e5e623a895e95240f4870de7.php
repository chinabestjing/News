...
<div id="right-content">
    <div class="top">
        <h2>用户管理 <small>预约修改</small></h2>
    </div>

    <form action="<?php echo U('AdminUser/yuyueEdit');?>" method="post">
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                <tr>
                    ...
                    <td>
                        <!--id隐藏域-->
                        <input name="m_id" value="<?php echo ($data["m_id"]); ?>" type="hidden" />
                        <input type="text" class="form-control w400" name="m_name" value="<?php echo ($data["m_name"]); ?>" />
                    </td>
                </tr>
                <tr>
                    <td width="100">联系电话</td>
                    <td>
                        <input type="number" class="form-control w400" name="m_phone" value="<?php echo ($data["m_phone"]); ?>" />
                    </td>
                </tr>
                <tr>
                    <td>是否通过</td>
                    <td>
                        <input type="radio" name="m_state" <?php if(($data["m_state"]) == "0"): ?>checked <?php else: ?>''<?php endif; ?> value="0" />否
                        <input type="radio" name="m_state" <?php if(($data["m_state"]) == "1"): ?>checked <?php else: ?>''<?php endif; ?> value="1" />是
                    </td>
                </tr>
                <tr>
                    <td width="100">预约时间</td>
                    <td>
                        <input type="datetime" class="form-control w400" name="m_time" value="<?php echo (date('Y-m-d H:m:s',$data["m_time"])); ?>" />
                    </td>
                </tr>
                <tr>
                    <td></td>
